...
class Gallery extends Component {
  ...
  render() {
    ...
    const imageItem = imagesList.filter(each => each.id === imageId)
    const imgUrl = imageItem[0].imageUrl
    const imgAlt = imageItem[0].imageAltText
          ...
          <ul className="thumbnail-images-container">
            {imagesList.map(eachImage => (
              <ThumbnailItem
                ...
                onSelectThumbNail={this.onClickThumbNail}
                isSelected={eachImage.id === imageId}
              />
            ))}
          </ul>
        </div>
